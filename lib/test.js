...
export async function run(ta) {
	...
	for (let th of ah) {
		...
		for (let [id, n, a, e] of at) {
			...
			if (e && id === "yaml") {
				// todo: waiting for
				// Shiki has an issue when it attempts to merge multiple grammars.
				// However, I can see that everything is highlighted correctly. I hope
